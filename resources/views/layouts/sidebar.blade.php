...
<aside class="fixed inset-y-0 left-0 bg-slate-900 text-slate-200 z-40 transform transition-all duration-300 lg:translate-x-0"
    ...
    <nav class="px-3 py-4 space-y-1 overflow-y-auto" style="max-height: calc(100vh - 4rem);">
        ...
        @endcan
        @can('ventas.ver')
            <a href="{{ route('admin.cuentas_cobrar.index') }}"
               class="flex items-center gap-3 px-3 py-2 rounded-lg transition group relative
                      {{ request()->routeIs('admin.cuentas_cobrar.*') ? 'bg-orange-600 text-white shadow-lg' : 'hover:bg-slate-800' }}"
               :class="sidebarCollapsed ? 'justify-center' : ''">
                <span class="text-xl flex-shrink-0">💳</span>
                <span x-show="!sidebarCollapsed" x-cloak class="text-sm font-medium">Cuentas por cobrar</span>
                <span x-show="sidebarCollapsed" x-cloak class="absolute left-full ml-2 px-2 py-1 bg-slate-800 text-white text-xs rounded shadow-lg opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50 transition">Cuentas por cobrar</span>
            </a>
        @endcan
        ...
        @can('transferencias.gestionar')
            <a href="{{ route('admin.transferencias.index') }}"
               class="flex items-center gap-3 px-3 py-2 rounded-lg transition group relative
                      {{ request()->routeIs('admin.transferencias.*') ? 'bg-orange-600 text-white shadow-lg' : 'hover:bg-slate-800' }}"
               :class="sidebarCollapsed ? 'justify-center' : ''">
                <span class="text-xl flex-shrink-0">🔄</span>
                <span x-show="!sidebarCollapsed" x-cloak class="text-sm font-medium">Transferencias</span>

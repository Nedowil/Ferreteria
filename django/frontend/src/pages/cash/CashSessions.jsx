          ...
          <tbody>
            {data.results.map((s) => (
              <tr key={s.id} className="border-t border-slate-100 hover:bg-slate-50/70 transition">
                ...
                <td className="px-4 py-2 font-medium text-slate-800">{s.user_name}</td>
                ...
                <td className="px-4 py-2 text-right font-semibold text-slate-700">Q{s.opening_amount}</td>
                <td className="px-4 py-2 text-right font-semibold text-slate-700">Q{s.expected_cash}</td>
                <td className="px-4 py-2 text-right font-semibold text-slate-700">{s.counted_cash != null ? `Q${s.counted_cash}` : "—"}</td>
                <td className={"px-4 py-2 text-right font-semibold " + (Number(s.difference) < 0 ? "text-red-600" : Number(s.difference) > 0 ? "text-green-600" : "text-slate-700")}>
                  {s.status === "cerrada" ? `Q${s.difference}` : "—"}
                </td>
                <td className="px-4 py-2"><span className={"inline-block rounded-full px-2 py-0.5 text-xs font-medium " + (s.status === "abierta" ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-600")}>{s.status_display}</span></td>
              </tr>
            ))}

      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 space-y-5">
          <section className="bg-white dark:bg-slate-800 rounded-lg shadow p-5 text-sm grid grid-cols-2 gap-2">
            <div><span className="text-slate-500 dark:text-slate-400">Proveedor:</span> <b>{p.supplier_name}</b></div>
            <div><span className="text-slate-500 dark:text-slate-400">Fecha:</span> {p.date}</div>
            <div><span className="text-slate-500 dark:text-slate-400">Factura:</span> {p.invoice_number || "—"}</div>
            <div><span className="text-slate-500 dark:text-slate-400">Sucursal:</span> {p.branch_name || "—"}</div>
            {p.received_at && <div><span className="text-slate-500 dark:text-slate-400">Recibida:</span> {new Date(p.received_at).toLocaleString()}</div>}
          </section>
          ...
          <section className="bg-white dark:bg-slate-800 rounded-lg shadow overflow-hidden">
            ...
            <table className="w-full text-sm">
              ...
              <tfoot className="text-sm">
                <tr className="border-t"><td colSpan="4" className="px-4 py-1 text-right text-slate-500 dark:text-slate-400">Subtotal</td><td className="px-4 py-1 text-right">Q{p.subtotal}</td></tr>
                <tr><td colSpan="4" className="px-4 py-1 text-right text-slate-500 dark:text-slate-400">IVA</td><td className="px-4 py-1 text-right">Q{p.tax}</td></tr>
          ...
          {p.payments.length > 0 && (
            <section className="bg-white dark:bg-slate-800 rounded-lg shadow overflow-hidden">
              ...
              <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400 text-left"><tr><th className="px-4 py-2">Fecha</th><th className="px-4 py-2">Método</th><th className="px-4 py-2">Ref.</th><th className="px-4 py-2 text-right">Monto</th></tr></thead>
                <tbody>
                  {p.payments.map((pm) => (
                    <tr key={pm.id} className="border-t"><td className="px-4 py-2">{pm.date}</td><td className="px-4 py-2">{pm.payment_method}</td><td className="px-4 py-2 text-slate-500 dark:text-slate-400">{pm.reference || "—"}</td><td className="px-4 py-2 text-right">Q{pm.amount}</td></tr>
                  ))}
                </tbody>
              </table>
              </div>
          ...
          <section className="bg-white dark:bg-slate-800 rounded-lg shadow p-5">
            ...
            {p.status === "pendiente" && (
              <div className="space-y-2">
                ...
                {can("compras.cancelar") && <button onClick={() => act("cancel")} className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded px-4 py-2 text-sm">Cancelar compra</button>}
                ...
                {!can("compras.recibir") && !can("compras.cancelar") && <p className="text-sm text-slate-500 dark:text-slate-400">Sin acciones disponibles.</p>}
              </div>
            )}
            {p.status !== "pendiente" && <p className="text-sm text-slate-500 dark:text-slate-400">Sin acciones disponibles.</p>}
          </section>
          ...
          {p.status === "recibida" && (
            <section className="bg-white dark:bg-slate-800 rounded-lg shadow p-5">
              ...
              <div className="text-sm mb-3">
                <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400">Pagado</span><span>Q{p.amount_paid}</span></div>
              ...
              {hasBalance ? (
                <form onSubmit={submitPay} className="space-y-2">
                  ...
                  <input placeholder="Referencia (opcional)" value={pay.reference} onChange={(e) => setPay({ ...pay, reference: e.target.value })} className="w-full border border-slate-300 dark:border-slate-600 rounded px-3 py-2 text-sm" />

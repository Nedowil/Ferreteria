  ...
  return (
    <div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); load(); }} className="bg-white rounded-xl shadow-sm border border-slate-100 p-4 mb-4 flex flex-wrap gap-2 items-end">
        ...
        {(filters.from || filters.to) && <button type="button" onClick={() => { setFilters({ from: "", to: "" }); api.get("/purchases/payable/").then((r) => setData(r.data)); }} className="text-sm text-slate-500 px-2 py-2 hover:underline">Limpiar</button>}
      </form>

  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => (onClose ? onClose() : navigate("/ventas"))} className="inline-flex items-center gap-1 text-sm font-medium text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg px-4 py-2 shadow-sm hover:bg-slate-50 hover:border-slate-400 transition">{onClose ? "Cerrar ✕" : "← Volver"}</button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 space-y-5">
          <section className="bg-white dark:bg-slate-800 rounded-lg shadow p-5 text-sm grid grid-cols-2 gap-2">
            <div><span className="text-slate-500 dark:text-slate-400">Cliente:</span> <b>{s.customer_name || "Consumidor final"}</b></div>
            <div><span className="text-slate-500 dark:text-slate-400">Fecha:</span> {new Date(s.date).toLocaleString()}</div>
            <div><span className="text-slate-500 dark:text-slate-400">Método:</span> {s.payment_method}</div>
            <div><span className="text-slate-500 dark:text-slate-400">Sucursal:</span> {s.branch_name || "—"}</div>
            {isAdmin && <div><span className="text-slate-500 dark:text-slate-400">Vendedor:</span> <b>{s.user_name || "—"}</b></div>}
          </section>
          ...
          <section className="bg-white dark:bg-slate-800 rounded-lg shadow overflow-hidden">
            ...
            <table className="w-full text-sm">
              ...
              <tfoot className="text-sm">
                <tr className="border-t"><td colSpan="3" className="px-4 py-1 text-right text-slate-500 dark:text-slate-400">Subtotal</td><td className="px-4 py-1 text-right">Q{s.subtotal}</td></tr>
                {Number(s.discount) > 0 && <tr><td colSpan="3" className="px-4 py-1 text-right text-slate-500 dark:text-slate-400">Descuento</td><td className="px-4 py-1 text-right">−Q{s.discount}</td></tr>}
                <tr><td colSpan="3" className="px-4 py-1 text-right text-slate-500 dark:text-slate-400">IVA</td><td className="px-4 py-1 text-right">Q{s.tax}</td></tr>
          ...
          {s.payments.length > 0 && (
            <section className="bg-white dark:bg-slate-800 rounded-lg shadow overflow-hidden">
              ...
              <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400 text-left"><tr><th className="px-4 py-2">Fecha</th><th className="px-4 py-2">Método</th><th className="px-4 py-2 text-right">Monto</th></tr></thead>
        ...
        <div className="space-y-5">
          <section className="bg-white dark:bg-slate-800 rounded-lg shadow p-5">
            ...
            <div className="text-sm space-y-1">
              <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400">Total</span><span>Q{s.total}</span></div>
              <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400">Recibido</span><span>Q{s.paid_amount}</span></div>
              {Number(s.change_amount) > 0 && <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400">Vuelto</span><span>Q{s.change_amount}</span></div>}
          ...
          <section className="bg-white dark:bg-slate-800 rounded-lg shadow p-5 space-y-3">

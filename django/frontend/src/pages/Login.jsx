...
export default function Login() {
  const { login, user, needsProfile } = useAuth();
  ...
  // Si ya hay sesión (perfil activo) o sesión de equipo, redirige.
  useEffect(() => {
    if (user) navigate("/", { replace: true });
    else if (needsProfile) navigate("/perfiles", { replace: true });
  ...
  const submit = async (e) => {
    ...
    try {
      const { isDevice } = await login(email, password);
      navigate(isDevice ? "/perfiles" : "/", { replace: true });
    } catch (err) {

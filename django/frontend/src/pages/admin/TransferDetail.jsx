  ...
  return (
    <div className="max-w-3xl">
      ...
      <section className="bg-white dark:bg-slate-800 rounded-lg shadow p-5 text-sm grid grid-cols-2 gap-2 mb-5">
        ...
        <div><span className="text-slate-500 dark:text-slate-400">Destino:</span> <b>{t.to_branch_name}</b></div>
        <div><span className="text-slate-500 dark:text-slate-400">Creada por:</span> {t.user_name || "—"}</div>
        <div><span className="text-slate-500 dark:text-slate-400">Fecha:</span> {new Date(t.date).toLocaleString()}</div>
        {t.sent_at && <div><span className="text-slate-500 dark:text-slate-400">Enviada:</span> {new Date(t.sent_at).toLocaleString()} ({t.sent_by_name})</div>}
        {t.received_at && <div><span className="text-slate-500 dark:text-slate-400">Recibida:</span> {new Date(t.received_at).toLocaleString()} ({t.received_by_name})</div>}
        {t.reason_cancelled && <div className="col-span-2 text-red-600"><span className="text-slate-500 dark:text-slate-400">Cancelada:</span> {t.reason_cancelled}</div>}
      </section>

      <section className="bg-white dark:bg-slate-800 rounded-lg shadow overflow-hidden mb-4">
        <div className="px-5 py-3 border-b font-semibold">Productos</div>
        <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400 text-left"><tr><th className="px-4 py-2">Producto</th><th className="px-4 py-2 text-right">Cantidad</th><th className="px-4 py-2 text-right">Base</th></tr></thead>
          <tbody>
            {t.items.map((it) => (
              <tr key={it.id} className="border-t">
                ...
                <td className="px-4 py-2 text-right text-slate-500 dark:text-slate-400">{it.quantity_base}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="flex gap-2">
        {t.status === "pendiente" && <>
          ...
          <button onClick={() => act("cancel")} className="bg-white dark:bg-slate-800 border border-red-300 text-red-600 rounded px-5 py-2 text-sm">Cancelar</button>
        </>}
        {t.status === "en_transito" && <>
          ...
          <button onClick={() => act("cancel")} className="bg-white dark:bg-slate-800 border border-red-300 text-red-600 rounded px-5 py-2 text-sm">Cancelar (devuelve stock)</button>
        </>}
      </div>
    </div>
  );

    ...
    <form onSubmit={submit} className="max-w-3xl space-y-5">
      ...
      <section className="bg-white rounded-lg shadow p-5 grid grid-cols-2 gap-4">
        <div>
          ...
          <select value={header.from_branch_id} onChange={(e) => setHeader({ ...header, from_branch_id: e.target.value })} className="w-full border border-slate-300 rounded px-3 py-2 text-sm">
        ...
        <div>
          ...
          <select value={header.to_branch_id} onChange={(e) => setHeader({ ...header, to_branch_id: e.target.value })} className="w-full border border-slate-300 rounded px-3 py-2 text-sm">
            <option value="">— Selecciona —</option>
            {branches.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}
          </select>
        </div>
      ...
      <section className="bg-white rounded-lg shadow p-5">
        ...
        <div className="relative mb-3">
          <input placeholder="Buscar producto…" value={search} onChange={(e) => doSearch(e.target.value)} className="w-full border border-slate-300 rounded px-3 py-2 text-sm" />
          {results.length > 0 && (
          ...
          <tbody>
            {items.map((it, idx) => (
              <tr key={idx} className="border-t">
                ...
                <td className="py-2"><input type="number" step="any" value={it.quantity_input} onChange={(e) => upd(idx, "quantity_input", e.target.value)} className="border border-slate-300 rounded px-2 py-1 text-sm w-24 text-right" /></td>
                <td className="py-2"><input type="number" step="any" value={it.units_factor} onChange={(e) => upd(idx, "units_factor", e.target.value)} className="border border-slate-300 rounded px-2 py-1 text-sm w-20 text-right" /></td>
                <td className="py-2"><input value={it.unit_label} onChange={(e) => upd(idx, "unit_label", e.target.value)} className="border border-slate-300 rounded px-2 py-1 text-sm w-24" /></td>
      ...
      <div className="flex gap-2">
        ...
        <button type="button" onClick={() => navigate("/transferencias")} className="px-6 py-2 text-slate-500">Cancelar</button>
      </div>
    </form>
  );
}

  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-900 px-4">
      <div className="w-full max-w-sm bg-white dark:bg-slate-800 rounded-xl shadow-lg p-8">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">Te enviaremos un enlace a tu correo.</p>
        {sent ? (
          <>
            <div className="bg-green-100 text-green-800 text-sm rounded px-3 py-2 mb-4">
              Si el correo está registrado, recibirás un enlace para restablecer tu contraseña.
            </div>
            <Link to="/login" className="text-sm text-slate-600 dark:text-slate-300 hover:text-slate-900">← Volver al inicio</Link>
          </>
        ) : (
          <form onSubmit={submit}>
            ...
            <div className="text-center mt-4">
              <Link to="/login" className="text-sm text-slate-500 dark:text-slate-400 hover:text-slate-800">← Volver al inicio</Link>
            </div>
          </form>
        )}
      </div>

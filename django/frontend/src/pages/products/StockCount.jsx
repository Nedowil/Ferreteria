...
export default function StockCount() {
  ...
  return (
    <div>
      <h1 className="text-lg font-semibold mb-4">Conteo físico</h1>
      ...
      <form onSubmit={submit}>
        <div className="bg-white rounded-lg shadow p-4 mb-4 flex items-center gap-4">
          ...
          <button disabled={busy || !changed.length} className="bg-blue-600 text-white rounded px-5 py-2 text-sm font-medium disabled:opacity-50">Aplicar conteo</button>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-slate-500 text-left">
              <tr><th className="px-4 py-2">SKU</th><th className="px-4 py-2">Producto</th>
                  <th className="px-4 py-2 text-right">Sistema</th><th className="px-4 py-2 text-right">Conteo físico</th>
                  <th className="px-4 py-2 text-right">Diferencia</th></tr>
            </thead>
            <tbody>
              {products.map((p) => {

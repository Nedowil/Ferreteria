  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4"
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center" onClick={(e) => e.stopPropagation()}>
        {icon}
        {state.title && <div className="text-lg font-bold text-slate-800 dark:text-slate-100 mt-3">{state.title}</div>}
        <div className={"whitespace-pre-line break-words " + (icon ? "mt-3" : "") + " " + (state.title ? "text-sm mt-1 text-slate-500 dark:text-slate-400" : "text-base font-semibold text-slate-700 dark:text-slate-200")}>
          {state.message}
        </div>
        {state.type === "prompt" && (
          <input autoFocus value={text} onChange={(e) => setText(e.target.value)}
                 onKeyDown={(e) => { if (e.key === "Enter") accept(); if (e.key === "Escape") cancel(); }}
                 className="w-full border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 rounded-lg px-3 py-2.5 text-sm mt-4 outline-none focus:ring-2 focus:ring-blue-500 text-left" />
        )}
        <div className="flex gap-3 mt-6">
          {state.type !== "alert" && (
            <button onClick={cancel} className="flex-1 px-4 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-600 transition">
